...
class TensorCPEncoding(Encoding):
    ...
    def forward(self, in_tensor: TensorType["bs":..., "input_dim"]) -> TensorType["bs":..., "output_dim"]:
        ...
        #line_grid: line_coord [3, 196608, 1, 2] (N:3,Hout:196608,Wout:1,2)
        #input:self.line_coef  torch.Size([3, 24, 256, 1])  (N:3,C:24,Hin:256,Win:1)  Hin is the resolution
        #output:line_features  (N:3,C:24,Hout:196608,Wout:1) 
...
'''
from the vqad tak:
there is a problem here because the index operation is non-differentiable, so you can't learn it in the differentiable set up.
To solve this issue,let's think about why this process is not differentiable 
when you query from this grid, you first get a integer as an index and the index is not differentiable 
here is the trick of the solution:
instead of think of this as an integer, we can think this as a binary vector that performs the matrixing through matrix multiplication, this vector now is binary
however, the pipeline is still not differentiable, we further replace this bianry vector with the max of the continous floating points values
replace the backward pass with the gradient of the soft max function to make it differentiable
...
class CodeBookEncoding(Encoding):    
    def __init__(
        ...
        self.dictioanry=torch.randn(size=(self.dictionary_size( * num_levels, self.bitwidth)) #* 2 - 1
    ...
    def _index_features(self, feats, idx, lod_idx):
        #how to get the feat(change the intger into the 16-bit vector)
        #how to build the dictionary and update the dictionary with the backward pass
        #now we can cntroil the code books(dctioanry) size
    ...
    def pytorch_fwd(self, in_tensor: TensorType["bs":..., "input_dim"]) -> TensorType["bs":..., "output_dim"]:
        ...
        #replace the hashed_i integer with the bianry vector and then  choose the max and then choosethe
